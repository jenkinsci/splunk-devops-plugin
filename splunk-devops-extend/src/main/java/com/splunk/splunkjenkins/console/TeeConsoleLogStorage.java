...
import static com.splunk.splunkjenkins.TeeConsoleLogFilter.TeeOutputStream;
...
public class TeeConsoleLogStorage implements LogStorage {
    ...
    private static final Map<File, TeeConsoleLogStorage> openStorages = Collections.synchronizedMap(new HashMap<>());

    public static synchronized TeeConsoleLogStorage forFile(File log) {
        return openStorages.computeIfAbsent(log, TeeConsoleLogStorage::new);
    }
    ...
    private TeeConsoleLogStorage(File log) {
        this.log = log;
    ...
    @Override public BuildListener overallListener() throws IOException, InterruptedException {
        return new BufferedBuildListener(new TeeOutputStream(new IndexOutputStream(null), source));

    }

    @Override public TaskListener nodeListener(FlowNode node) throws IOException, InterruptedException {
        return new BufferedBuildListener(new TeeOutputStream(new IndexOutputStream(node.getId()), source));
    }
...
//        private ByteArrayOutputStream2 buffer = new ByteArrayOutputStream2(RECEIVE_BUFFER_SIZE);
        ...
        @Override public void write(int b) throws IOException {
            synchronized (TeeConsoleLogStorage.this) {
                checkId(id);
                bos.write(b);
//                buffer.write(b);
//                if (b == LF || buffer.size() > CONSOLE_TEXT_SINGLE_LINE_MAX_LENGTH) {
//                    eol();
//                }
            }
        }
